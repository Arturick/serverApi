...
class PostService {
    ...
    try {
      ...
      switch (postType) {
        ...
        case "Category":
          ...
          categoriesObject = categoriesObject.sort((a, b) => a.order > b.order ? -1 : 1);
    ...
    try {
      switch (postType) {
        case "Category":
          ...
          break;
        case "Product":
          const product = await Product.find({id: post.id});
          console.log(product)
  ...
  async getCategoryFilter(categoryTitle, filters){
    try{
      const parsedFilters = filters.split(',');
  ...
  async getMainPageProducts(){
    try{
      let productList = await Product.find({displayMain: true});
      let doneList = [];
